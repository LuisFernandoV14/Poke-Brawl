...
//	system("pause"); system("cls"); //limpa a tela pra ficar bunitinho
	...
	printf("Vamos começar pelo seu nome: ");
	fflush(stdin);
	scanf("%s", treinador->nome);
	
	treinador->dinheiro = 1000; //inicializa o dinheiro inicial do jogador, (1000 é um valor temporário)
...
void pokedex(pkmn pkdex[])
{
	pkmn *listadepokemons = (pkmn *)malloc(sizeof(int) * 6); //tamanho provisório pq eu n sei quantos pokemon a gente tem lol
	int i;
...
void adicionarNaPokedex(pkmn pokemon, pkmn pkdex[])
{
	...
	for(i = 0; i < 24; i++) //tamanho provisório pq eu n sei quantos pokemon a gente tem lol	
	{
		if(pkdex[i].dex == 0) 
		{ 
...
void printarpkmn(pkmn pokemon, pkmn vetor[])
{
	...
	for(i = 0; i < 24; i++)
	{
		...
		{
			printf("O pokemon na posição %d é: %s \n", pokemon.dex, pokemon.nome);
			fflush(stdout);
		}
	}
